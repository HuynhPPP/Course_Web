    ...
    <div class="col-xl-12 mx-auto">
        <div class="card">
            <div class="card-body p-4">
                ...
                <form id="AddCategoryForm" action="{{ route('instructor.update.coupon') }}" method="post" class="row g-3" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select name="course_id" class="form-select mb-3" aria-label="Default select example">
                            <option selected="">Choose course apply coupon</option>
                            @foreach ($courses as $course)
                                <option value="{{ $course->id }}" 
                                               {{ $course->id ==  $coupon->course_id ? 'selected' : ''}}
                                >
                                    {{ $course->course_name }}
                                </option>
                            @endforeach
